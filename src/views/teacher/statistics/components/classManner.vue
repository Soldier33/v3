...
import { getData, getOption, getCurrentWeek} from "/@/api/teacher/statistics/classManner";
    ...
    const getMyData = () => {
      ...
      getData(state.searchValue).then((res) => {
        ...
        if (res.result.data && res.result.data.length )  {
            ...
            const weekLen = res.result.data[0].week.length
            // state.headData = []
            for (let i = 0; i < weekLen; i++) {
              state.headData.push({ text: `周${i+1}`, value: `week`, index: i})
            }
